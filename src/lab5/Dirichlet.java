...
//            a[7][4] = 36.0;
        ...
        for (int count = 0; count < 100; count++) {
            ...
            if (rank > 0) {
                MPI.COMM_WORLD.Sendrecv(b, rank * strNum, 1, MPI.OBJECT, rank - 1, 0,
                        a, rank * strNum - 1, 1, MPI.OBJECT, rank - 1, 0);
            }

            if (rank < size - 1) {
                MPI.COMM_WORLD.Sendrecv(b, (rank + 1) * strNum - 1, 1, MPI.OBJECT, rank + 1, 0,
                        a, (rank + 1) * strNum, 1, MPI.OBJECT, rank + 1, 0);
            }

        }

        MPI.COMM_WORLD.Gather(a, rank * strNum, strNum, MPI.OBJECT, c, 0, strNum, MPI.OBJECT, 0);
